...
DARK_GREEN = RGBColor(0x35, 0x61, 0x58)
...
def build_pptx(logo_path: str, selected_period: str, data_range: str, metrics: list, narrative: list, sections: list) -> bytes:
    """
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        _add_textbox(slide, 0, 2.8, SLIDE_W_IN, 1, "Операционный отчёт клиники", 34, DARK_GREEN, bold=True, align=PP_ALIGN.CENTER)
        _add_textbox(slide, 0, 3.7, SLIDE_W_IN, 0.6, f"Период: {selected_period}", 18, GRAY_TEXT, align=PP_ALIGN.CENTER)
        _add_textbox(slide, 0, 4.2, SLIDE_W_IN, 0.5, f"Данные: {data_range}", 11, GRAY_TEXT, italic=True, align=PP_ALIGN.CENTER)

        # ── Сводка: метрики + тезисы ──
        slide = prs.slides.add_slide(blank_layout)
        _set_cream_background(slide)
        _add_logo(slide, logo_path)
        _add_textbox(slide, 0.4, 0.5, 10, 0.6, f"Главное за период: {selected_period}", 22, DARK_GREEN, bold=True)

        col_w = (SLIDE_W_IN - 0.8) / max(len(metrics), 1)
        for i, (label, value, delta) in enumerate(metrics):
            x = 0.4 + i * col_w
            _add_textbox(slide, x, 1.3, col_w - 0.1, 0.5, label, 10, GRAY_TEXT)
            _add_textbox(slide, x, 1.7, col_w - 0.1, 0.5, value, 16, DARK_GREEN, bold=True)
            if delta:
                color = RED_TEXT if delta.startswith("-") else GREEN_TEXT
                _add_textbox(slide, x, 2.15, col_w - 0.1, 0.4, delta, 10, color)

        _add_textbox(slide, 0.4, 2.8, 10, 0.5, "Главные тезисы периода", 16, DARK_GREEN, bold=True)
        y = 3.3
        for heading, text in narrative:
            _add_textbox(slide, 0.4, y, 12.5, 0.3, heading, 12, DARK_GREEN, bold=True)
            y += 0.32
        ...
        for i, section in enumerate(sections):
            ...
            if section["tab"] != current_tab:
                ...
                _add_textbox(slide, 0.4, 0.5, 12, 0.6, current_tab, 22, DARK_GREEN, bold=True)
                y_cursor = 1.3
            else:
                y_cursor = 0.5

            _add_textbox(slide, 0.4, y_cursor, 12.5, 0.6, section["heading"], 13, DARK_GREEN, bold=True)
